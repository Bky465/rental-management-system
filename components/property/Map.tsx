...
export const Map = ({ ...pageProps }) => {
  ...
  const myLat = parseFloat(properties && properties[0].lat) || null;
  const myLng = parseFloat(properties && properties[0].lng) || null;
  ...
  const price = myproperty?.bookingPricing.expectedPrice
    ? validatePrice(myproperty?.bookingPricing.expectedPrice)
    : myproperty?.bookingPricing.monthlyRental
    ? validatePrice(myproperty?.bookingPricing.monthlyRental)
    : "N/A";
  return isLoaded ? (
    <div className="relative">
      ...
      {myproperty && (
        <div
          className="absolute top-0 right-0 h-auto bg-white"
        >
          <EnquiryForm
            isOpen={open}
            setIsOpen={setOpen}
          ...
          <div className="xs:w-[450px] w-[300px] h-full  shadow-md flex flex-col rounded-lg">
            <div className="h-auto  w-full relative ">
              ...
              {/* {<div className="absolute bottom-0 right-0 flex m-2"></div>} */}
            </div>
            ...
            <div className=" mt-[0.2rem] xxs:mt-2 mb-3">
              <div className="px-2 lg1:px-4">
                ...
                {/* <p className="text-lg font-semibold text-primaryColor mt-1">
                  ₹
                  {myproperty?.bookingPricing?.expectedPrice
                    ? `${myproperty.bookingPricing?.expectedPrice / 100000} Lac`
                    : myproperty.bookingPricing?.deposit
                    ? `${myproperty.bookingPricing?.deposit / 100000} Lac`
                    : "N/A"}{" "}
                  {myproperty?.bookingPricing?.maintenanceCharges2 && (
                    <span className="text-sm">
                      + {myproperty?.bookingPricing?.maintenanceCharges2 / 1000}{" "}
