...
const Index = ({ ...pageProps }) => {
  ...
  const fetchProperty = async (id: any) => {
    try {
      ...
      if (response.success) {
        ...
        const data = response.gallery.filter((item:any)=> ["brochure","floorPlan"].indexOf(item.category) === -1);
        const mydata = data.filter((item:any)=> item.category !== "video");
        setMyGallery(mydata);
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      getReviews({id: router.query.id}).then((response:any)=> {
        if(response.success) {
  ...
  return (
    <>
      ...
      {/* <ScrollToTop WindowWidth={pageProps.windowWidth} /> */}

      {isLoading && <LoadingSpinner />}

      <ScrollToTop scrollToTop={pageProps.scrollToTop} />
      <div className={`fixed top-75px left-0 w-full pt-4 z-[50] bg-white ${shadow ? "shadow-nav top-bg-color" : ""}`}>
        <button
          onClick={() => Router.back()}
      ...
      <div className="w-full mt-[185px] xs:mt-[170px] sm:mt-[140px] lg:mt-[150px] lg1:mt-[150px] 2xl:mt-[170px] px-6 sm:px-12 base:px-16">
        ...
        <div ref={floorplanref} className="mt-20  ">
          <FloorPlan property={property} gallery={gallery} />
        </div>
        <div ref={reviewref} className="mt-20  ">
          <About property={property} gallery={mygallery} />
        </div>
